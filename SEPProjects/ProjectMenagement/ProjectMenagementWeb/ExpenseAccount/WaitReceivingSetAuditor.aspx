<%@ Page Title="" Language="C#" MasterPageFile="~/MasterPage.master" AutoEventWireup="true"
    Inherits="ExpenseAccount_WaitReceivingSetAuditor" EnableEventValidation="false" CodeBehind="WaitReceivingSetAuditor.aspx.cs" %>
...
<asp:Content ID="Content1" ContentPlaceHolderID="ContentPlaceHolder1" runat="Server">
    ...
    <script language="javascript">
        ...
        function getInsertIndex(type) {
            ...
            for (i = 1; i < table.rows.length; i++) {
                if (table.rows[i].cells[4].innerHTML == getAuditorTypeName("YS")) {
                    ...
                    zjlfjIndex = i;
                } else if (table.rows[i].cells[4].innerHTML == getAuditorTypeName("CWZJ")) {
                    cwzjIndex = i;
                }
            }
            if (type == "YS") {
                ...
                if (cwzjIndex == 0)  { if (zjlfjIndex == 0) { if (zjlzhIndex == 0) { if (zjlspIndex == 0) { if (zjfjIndex == 0) { if (zjzhIndex == 0) { if (zjspIndex == 0) { insertIndex = ysIndex; } else { insertIndex = zjspIndex; } } else { insertIndex = zjzhIndex; } } else { insertIndex = zjfjIndex } } else { insertIndex = zjlspIndex; } } else { insertIndex = zjlzhIndex; } } else { insertIndex = zjlfjIndex; } } else { insertIndex = cwzjIndex; }
            }
        ...
        function addUser(type, userId) {
            ...
            if (type != "ZJZH" && type != "ZJLZH") {
                ...
                hidCWZJ = document.getElementById("<%=hidCWZJ.ClientID %>").value;
                if (!checkExist(hidValue, userId) && !checkExist(hidYS, userId) && !checkExist(hidZJFJ, userId) && !checkExist(hidZJSP, userId) && !checkExist(hidZJLSP, userId) && !checkExist(hidZJLFJ, userId) && !checkExist(hidCWZJ, userId)) {
        ...
        function openEmployee(type) {
            ...
            var win = window.open('selectOperationAuditor2.aspx?<% =ESP.Finance.Utility.RequestName.ReturnID %>=<%=Request[ESP.Finance.Utility.RequestName.ReturnID] %>&curU=' + currentUser + "&type=" + type, null, 'height=600px,width=800px,scrollbars=yes,top=100px,left=100px');
            win.resizeTo(screen.availWidth * 0.8, screen.availHeight * 0.8);
        }
        function openEmployee1(type, changedTrId, oldId) {
            var currentUser = '<%= CurrentUser.SysID %>';
            var win = window.open('selectOperationAuditor2.aspx?<% =ESP.Finance.Utility.RequestName.ReturnID %>=<%=Request[ESP.Finance.Utility.RequestName.ReturnID] %>&changedTrId=' + changedTrId + '&oldId=' + oldId + '&type=' + type + '&curU=' + currentUser, null, 'height=600px,width=800px,scrollbars=yes,top=100px,left=100px');
            win.resizeTo(screen.availWidth * 0.8, screen.availHeight * 0.8);
        }
    </script>
    ...
    <asp:HiddenField ID="hidCWZJ" runat="server" />
    ...
    <table border="0" width="100%">
        <tr>
            <td align="center">
                <prc:CheckPRAspButton ID="btnSave" runat="server" Text=" 保存 " CssClass="widebuttons" OnClick="btnSave_Click" Visible="false" />&nbsp;
                <prc:CheckPRInputButton runat="server" id="btnCommit" value=" 提交 " type="button" onclick="showLoading();"
