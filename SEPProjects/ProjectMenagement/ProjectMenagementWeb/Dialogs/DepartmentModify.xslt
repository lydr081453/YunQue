...
<xsl:template match="/">
  <ul STYLE="DISPLAY:block">
    ...
    <ul STYLE="DISPLAY:block"><xsl:apply-templates select="recordset/row"><xsl:sort select="@sort" /></xsl:apply-templates></ul>
    </li>
  </ul>
</xsl:template>
...
<xsl:template match="row">
	<li>		
		...
		<xsl:if test="row"><ul><xsl:apply-templates><xsl:sort select="@sort" /></xsl:apply-templates></ul></xsl:if>
	</li>
</xsl:template>
