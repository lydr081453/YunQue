...
{
    public partial class RemindAudit : ESP.Purchase.WebPage.ViewPageForPR
    {
        int generalid = 0;
        ...
        public void BindInfo()
        {
            ...
            if (null != g)
            {
                ...
                if (!string.IsNullOrEmpty(Request["RecipientId"]))
                    detailTerm = " OrderID=" + Request["RecipientId"] + " and ordertype="+(int)ESP.Purchase.Common.State.RemindOrderType.Payment;
                else if (string.IsNullOrEmpty(Request["RecipientId"]) && !string.IsNullOrEmpty(Request[RequestName.GeneralID]))
                {
                    detailTerm = " OrderID=" + Request[RequestName.GeneralID] + " and ordertype=" + (int)ESP.Purchase.Common.State.RemindOrderType.Recipient;
                    litTitle.Text = "收货延迟信息";
                }

                gvList.DataSource = BlackDetailManager.GetList(detailTerm,new List<SqlParameter>());
                gvList.DataBind();
        ...
        {
            Response.Redirect(Request["backUrl"] == null ? "RemindPNList.aspx" : Request["backUrl"].ToString());
        }
