<%@ Page Title="业务审批人" Language="C#" MasterPageFile="~/MasterPage.master" AutoEventWireup="true" EnableEventValidation="false" Inherits="Purchase_Requisition_selectOperationAuditor" Codebehind="selectOperationAuditor.aspx.cs" %>
...
<script type="text/javascript">
    $().ready(function() {
        ...
        Purchase_Requisition_selectOperationAuditor.getalist($("#<%=hidtype.ClientID %>").val(), init1);
        function init1(r) {
            if (r.value != null)
        ...
        Purchase_Requisition_selectOperationAuditor.getalist($("#<%=hidtype1.ClientID %>").val(), init2);
        function init2(r) {
            $("#<%=ddltype2.ClientID %>").empty();
        ...
        $("#<%=ddltype.ClientID %>").change(function() {
            ...
            Purchase_Requisition_selectOperationAuditor.getalist($("#<%=ddltype.ClientID %>").val(), pop1);
            function pop1(r) {
                if (r.value != null)
        ...
        $("#<%=ddltype1.ClientID %>").change(function() {
            ...
            Purchase_Requisition_selectOperationAuditor.getalist($("#<%=ddltype1.ClientID %>").val(), pop2);
            function pop2(r) {
                if (r.value != null)
                    for (i = 0; i < r.value.length; i++) {
    ...
    <table width="100%" class="tableForm">
        ...
        <tr>
            <td class="oddrow">审批角色：<asp:HiddenField ID="hidIds" runat="server" /></td>
...
<%--        <tr>
        <td class="oddrow">知会人类型：</td>
            <td class="oddrow-l"><asp:RadioButtonList ID="radZH" runat="server" AutoPostBack="true" RepeatDirection="Horizontal" CssClass="XTable" OnSelectedIndexChanged="radZH_SelectIndexChanged" /></td>
            <td class="oddrow">审批人类型：</td><asp:HiddenField ID="hidIds" runat="server" />
            <td class="oddrow-l"><asp:RadioButtonList ID="radSP" runat="server"  AutoPostBack="true" RepeatDirection="Horizontal" CssClass="XTable" OnSelectedIndexChanged="radSP_SelectIndexChanged" /></td>--%>
            <%--<asp:RadioButtonList ID="ddlAuditorType" runat="server"  BorderStyle="None" RepeatDirection="Horizontal" AutoPostBack="true" OnSelectedIndexChanged="ddlAuditorType_SelectIndexType" />--%>
            <%--<asp:DropDownList ID="ddlAuditorType" runat="server" AutoPostBack="true" OnSelectedIndexChanged="ddlAuditorType_SelectIndexType" />--%><%--</td>
        </tr>--%>
        <tr>
