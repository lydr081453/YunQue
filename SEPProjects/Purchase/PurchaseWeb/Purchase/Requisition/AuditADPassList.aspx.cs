...
{
    public partial class AuditADPassList : ESP.Web.UI.PageBase
    {
        ...
        private void ListBind()
        {
            ...
            term = " and (status = @status or status = @status_sended  or status = @confirm or status = @recipiented ) and prtype =3 ";
        ...
        protected void gvG_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            ...
            if (g.InUse == (int)State.PRInUse.Use)
            {
                ...
                if (e.CommandName == "Return")
                {
                    ...
                    ClientScript.RegisterStartupScript(typeof(string), "", "alert('申请单撤销成功!');window.location='AuditADPassList.aspx'", true);

                }
            }
            else
            {
                ClientScript.RegisterStartupScript(typeof(string), "", "alert('" + ESP.Purchase.Common.State.DisabledMessageForPR + "');window.location='AuditADPassList.aspx'", true);
            }
        }
        ...
        protected void HandConfirm(GeneralInfo generalInfo)
        {
            ...
            ClientScript.RegisterStartupScript(typeof(string), "", "window.location.href='AuditADPassList.aspx';alert('已完成供应商确认！"+exMail+"');", true);
        }
        ...
        protected void gvG_RowDataBound(object sender, GridViewRowEventArgs e)
        {
            ...
            if (e.Row.RowType == DataControlRowType.DataRow)
            {
                ...
                if (null != hypAuditurl)
                {
                    hypAuditurl.NavigateUrl = "OrderDetail.aspx?backUrl=AuditADPassList.aspx&" + RequestName.GeneralID + "=" + e.Row.Cells[0].Text.ToString() + "&helpfile=3";
                    hypNoView.NavigateUrl = "OrderDetail.aspx?backUrl=AuditADPassList.aspx&" + RequestName.GeneralID + "=" + e.Row.Cells[0].Text.ToString() + "&helpfile=3";
                }
                ...
                {
                    if (model.status == State.order_ok || model.status == State.order_confirm || model.status == State.requisition_recipiented)
                    {
