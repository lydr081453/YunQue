<%@ Page Title="" Language="C#" MasterPageFile="~/MasterPage.master" AutoEventWireup="true"
    CodeBehind="RemindRecipientList.aspx.cs" Inherits="PurchaseWeb.Purchase.Requisition.RemindRecipientList" %>
                                        ...
                                        <asp:GridView ID="gvG" runat="server" AutoGenerateColumns="False" DataKeyNames="id"
                                            ...
                                            <Columns>
                                                <asp:TemplateField HeaderText="选择" ItemStyle-Width="5%" ItemStyle-HorizontalAlign="Center">
                                                    ...
                                                    <ItemTemplate>
                                                        <input type="checkbox" id="chkItem" runat="server" name="chkItem" onclick="checkOne(this);"
                                                            value='<% # Eval("id").ToString() + "#" + Eval("recipientDate").ToString() + "#" + Eval("RecipientNo") %>' />
                                                    </ItemTemplate>
                                                </asp:TemplateField>
                                                ...
                                                <asp:BoundField HeaderText="订单号" DataField="orderid" Visible="false" ItemStyle-Width="7%" ItemStyle-HorizontalAlign="Center" />
                                                ...
                                                <asp:TemplateField HeaderText="申请人" ItemStyle-HorizontalAlign="Center" ItemStyle-Width="6%" Visible="false">
                                                    <ItemTemplate>
                                                ...
                                                <asp:TemplateField HeaderText="收货人" ItemStyle-HorizontalAlign="Center" ItemStyle-Width="6%">
                                                    <ItemTemplate>
                                                        <span class="userLabel" onclick="ShowMsg('<%# Eval("goods_receiver").ToString() == "0" ? "" : ESP.Web.UI.PageBase.GetUserInfo(int.Parse(Eval("goods_receiver").ToString())) %>');">
                                                            <%# Eval("receivername")%></span>
                                                    </ItemTemplate>
                                                </asp:TemplateField>
                                                <asp:TemplateField HeaderText="收货时间" ItemStyle-HorizontalAlign="Center" ItemStyle-Width="8%">
                                                    <ItemTemplate>
                                                        <%# DateTime.Parse(Eval("recipientDate").ToString()) == DateTime.Parse(State.datetime_minvalue) ? "" : Eval("recipientDate").ToString() %>
                                                    </ItemTemplate>
                                                </asp:TemplateField>
                                                <asp:TemplateField HeaderText="附加收货人" ItemStyle-HorizontalAlign="Center" ItemStyle-Width="6%">
                                                    <ItemTemplate>
                                                        <span class="userLabel" onclick="ShowMsg('<%# Eval("appendReceiver").ToString() == "0" ? "" : ESP.Web.UI.PageBase.GetUserInfo(int.Parse(Eval("appendReceiver").ToString())) %>');">
                                                            <%# Eval("appendReceiverName")%></span>
                                                    </ItemTemplate>
                                                </asp:TemplateField>
                                                <asp:BoundField HeaderText="预计收货时间" DataField="EstimateDate" ItemStyle-HorizontalAlign="Center" ItemStyle-Width="8%"/>
                                                ...
                                                <asp:TemplateField HeaderText="邮件提醒" ItemStyle-HorizontalAlign="Center" ItemStyle-Width="5%">
                                                    <ItemTemplate>
                                                        <asp:LinkButton ID="lnkSend" runat="server" CommandArgument='<% # Eval("id").ToString() + "#" + Eval("EstimateDate").ToString()+"#" + Eval("RecipientNo")  %>'
                                                            Font-Underline="true" Text="<img title='发送邮件' src='../../images/Icon_Sendmail.gif' border='0' />"
                                                            CommandName="SendMail" />
                                                    </ItemTemplate>
                                                </asp:TemplateField>
                                                <asp:TemplateField HeaderText="反馈审核" ItemStyle-HorizontalAlign="Center" ItemStyle-Width="5%" Visible="false">
                                                    <ItemTemplate>
                                                        <asp:HyperLink ID="hylAudit" runat="server" ImageUrl="~/images/dc.gif" NavigateUrl='<%# "RemindAudit.aspx?backUrl=RemindRecipientList.aspx&" + ESP.Purchase.Common.RequestName.GeneralID + "=" + Eval("id") %>' />
                                                    </ItemTemplate>
                                                </asp:TemplateField>
                                            </Columns>
