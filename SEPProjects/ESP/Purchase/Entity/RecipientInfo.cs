...
namespace ESP.Purchase.Entity
{
    ...
    public class RecipientInfo
    {
        ...
        public void PopupData(IDataReader r)
        {
            ...
            if (r["Gid"] != DBNull.Value && r["Gid"].ToString() != "")
            {
                Gid = int.Parse(r["Gid"].ToString());
            }
            RecipientName = r["RecipientName"].ToString();
            Note = r["Note"].ToString();
            if (r["RecipientDate"] != DBNull.Value && r["RecipientDate"].ToString() != "")
            {
                RecipientDate = DateTime.Parse(r["RecipientDate"].ToString());
            }
            if (r["RecipientAmount"] != DBNull.Value && r["RecipientAmount"].ToString() != "")
            {
                RecipientAmount = decimal.Parse(r["RecipientAmount"].ToString());
            }
            if (r["Status"] != DBNull.Value && r["Status"].ToString() != "")
            {
                Status = int.Parse(r["Status"].ToString());
            }
            receivePrice = r["receivePrice"] == DBNull.Value ? 0 : int.Parse(r["receivePrice"].ToString());
            account_name = r["account_name"].ToString();
            account_bank = r["account_bank"].ToString();
            account_number = r["account_number"].ToString();
            IsConfirm = r["IsConfirm"] == DBNull.Value ? 0 : int.Parse(r["IsConfirm"].ToString());
            RecipientNo = r["RecipientNo"].ToString();
            AppraiseRemark = r["AppraiseRemark"].ToString();

            _singleprice = r["singleprice"].ToString();
            _num = r["num"].ToString();
            _des = r["des"].ToString();
        }
    }
