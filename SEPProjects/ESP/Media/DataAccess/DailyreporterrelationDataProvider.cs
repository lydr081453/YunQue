...
{
    public class DailyreporterrelationDataProvider
    {
        #region 构造函数
        public DailyreporterrelationDataProvider()
        {
        }
        #endregion
        #region 插入
        //插入字符串
        private static string strinsert(DailyreporterrelationInfo obj, ref List<SqlParameter> ht)
        {
            if (ht == null)
            {
                ht = new List<SqlParameter>();
            }
            string sql = @"insert into media_dailyreporterrelation (dailyid,reporterid,relationdate,relationuserid,remark,del,paytype,bankname,bankcardcode,bankcardname,bankacountname,writingfee,referral,haveinvoice,paystatus,financecode,uploadstarttime,uploadendtime,paymentmode,privateremark,cooperatecircs) values (@dailyid,@reporterid,@relationdate,@relationuserid,@remark,@del,@paytype,@bankname,@bankcardcode,@bankcardname,@bankacountname,@writingfee,@referral,@haveinvoice,@paystatus,@financecode,@uploadstarttime,@uploadendtime,@paymentmode,@privateremark,@cooperatecircs);select @@IDENTITY as rowNum;";
        ...
        //插入一条记录
        public static int insertinfo(DailyreporterrelationInfo obj, SqlTransaction trans)
        {
            if (obj == null)
            {
                return 0;
        ...
        //插入一条记录
        public static int insertinfo(DailyreporterrelationInfo obj)
        {
            if (obj == null)
            {
                return 0;
        ...
        {
            int rows = 0;
            string sql = "delete media_dailyreporterrelation where id=@id";
        ...
        #region 更新
        //更新sql
        public static string getUpdateString(DailyreporterrelationInfo objTerm, DailyreporterrelationInfo Objupdate, string term, ref List<SqlParameter> ht, params SqlParameter[] param)
        {
            string sql = "update media_dailyreporterrelation set dailyid=@dailyid,reporterid=@reporterid,relationdate=@relationdate,relationuserid=@relationuserid,remark=@remark,del=@del,paytype=@paytype,bankname=@bankname,bankcardcode=@bankcardcode,bankcardname=@bankcardname,bankacountname=@bankacountname,writingfee=@writingfee,referral=@referral,haveinvoice=@haveinvoice,paystatus=@paystatus,financecode=@financecode,uploadstarttime=@uploadstarttime,uploadendtime=@uploadendtime,paymentmode=@paymentmode,privateremark=@privateremark,cooperatecircs=@cooperatecircs where 1=1 ";
            SqlParameter param_dailyid = new SqlParameter("@dailyid", SqlDbType.Int, 4);
        ...
        //更新操作
        public static bool updateInfo(SqlTransaction trans, DailyreporterrelationInfo objterm, DailyreporterrelationInfo Objupdate, string term, params SqlParameter[] param)
        {
            if (Objupdate == null)
            {
                return false;
        ...
        //更新操作
        public static bool updateInfo(DailyreporterrelationInfo objterm, DailyreporterrelationInfo Objupdate, string term, params SqlParameter[] param)
        {
            if (Objupdate == null)
            {
                return false;
        ...
        private static string getTerms(DailyreporterrelationInfo obj, ref List<SqlParameter> ht)
        {
        ...
        #endregion
        #region 查询
        private static string getQueryTerms(DailyreporterrelationInfo obj, ref Hashtable ht)
        {
            string term = string.Empty;
            if (obj.Id > 0)//id
            {
        ...
        public static DataTable QueryInfoByObj(DailyreporterrelationInfo obj, string terms, Hashtable ht)
        {
            if (ht == null)
            {
                ht = new Hashtable();
        ...
        public static DataTable QueryInfoByObj(DailyreporterrelationInfo obj, string terms, params SqlParameter[] param)
        {
            if (terms == null)
            {
                terms = string.Empty;
        ...
        #endregion
        #region load
        public static DailyreporterrelationInfo Load(int id)
        {
            DataTable dt = null;
        ...
        public static DailyreporterrelationInfo Load(int id, SqlTransaction trans)
        {
            DataTable dt = null;
        ...
        public static DailyreporterrelationInfo setObject(DataRow dr)
        {
            DailyreporterrelationInfo obj = new DailyreporterrelationInfo();
            if (dr.Table.Columns.Contains("id") && dr["id"] != DBNull.Value)//id
            {
                obj.Id = Convert.ToInt32(dr["id"]);
            }
            if (dr.Table.Columns.Contains("dailyid") && dr["dailyid"] != DBNull.Value)//日常传播编号
            {
