...
{
    public class CashloanitemDataProvider
    {
        #region 构造函数
        public CashloanitemDataProvider()
        {
        }
        #endregion
        #region 插入
        //插入字符串
        private static string strinsert(CashloanitemInfo obj, ref List<SqlParameter> ht)
        {
            if (ht == null)
            {
                ht = new List<SqlParameter>();
            }
            string sql = @"insert into media_cashloanitem (cashloanbillid,describe,amount,del,projectid,projectcode,createip,createdate,userid,username,userdepartmentid,userdepartmentname,userextensioncode,reimbursedate) values (@cashloanbillid,@describe,@amount,@del,@projectid,@projectcode,@createip,@createdate,@userid,@username,@userdepartmentid,@userdepartmentname,@userextensioncode,@reimbursedate);select @@IDENTITY as rowNum;";
            SqlParameter param_Cashloanbillid = new SqlParameter("@Cashloanbillid", SqlDbType.Int, 4);
            param_Cashloanbillid.Value = obj.Cashloanbillid;
            ht.Add(param_Cashloanbillid);
        ...
        //插入一条记录
        public static int insertinfo(CashloanitemInfo obj, SqlTransaction trans)
        {
            if (obj == null)
            {
                return 0;
        ...
        //插入一条记录
        public static int insertinfo(CashloanitemInfo obj)
        {
            if (obj == null)
            {
                return 0;
        ...
        {
            int rows = 0;
            string sql = "delete media_cashloanitem where id=@id";
        ...
        #region 更新
        //更新sql
        public static string getUpdateString(CashloanitemInfo objTerm, CashloanitemInfo Objupdate, string term, ref List<SqlParameter> ht, params SqlParameter[] param)
        {
            string sql = "update media_cashloanitem set cashloanbillid=@cashloanbillid,describe=@describe,amount=@amount,del=@del,projectid=@projectid,projectcode=@projectcode,createip=@createip,createdate=@createdate,userid=@userid,username=@username,userdepartmentid=@userdepartmentid,userdepartmentname=@userdepartmentname,userextensioncode=@userextensioncode,reimbursedate=@reimbursedate where 1=1 ";
            SqlParameter param_cashloanbillid = new SqlParameter("@cashloanbillid", SqlDbType.Int, 4);
            param_cashloanbillid.Value = Objupdate.Cashloanbillid;
            ht.Add(param_cashloanbillid);
            SqlParameter param_describe = new SqlParameter("@describe", SqlDbType.NVarChar, 1000);
        ...
        //更新操作
        public static bool updateInfo(SqlTransaction trans, CashloanitemInfo objterm, CashloanitemInfo Objupdate, string term, params SqlParameter[] param)
        {
            if (Objupdate == null)
            {
                return false;
        ...
        //更新操作
        public static bool updateInfo(CashloanitemInfo objterm, CashloanitemInfo Objupdate, string term, params SqlParameter[] param)
        {
            if (Objupdate == null)
            {
                return false;
        ...
        private static string getTerms(CashloanitemInfo obj, ref List<SqlParameter> ht)
        {
            ...
            if (obj.Cashloanbillid > 0)//cashloanbillid
            {
                term += " and cashloanbillid=@cashloanbillid ";
                if (ESP.Media.Access.Utilities.Common.Find(ht, "@cashloanbillid") == -1)
                {
                    SqlParameter p = new SqlParameter("@cashloanbillid", SqlDbType.Int, 4);
                    p.Value = obj.Cashloanbillid;
                    ht.Add(p);
                }
            }
        ...
        #endregion
        #region 查询
        private static string getQueryTerms(CashloanitemInfo obj, ref Hashtable ht)
        {
            string term = string.Empty;
            if (obj.Id > 0)//id
            {
            ...
            if (obj.Cashloanbillid > 0)//cashloanbillid
            {
                term += " and a.cashloanbillid=@cashloanbillid ";
                if (!ht.ContainsKey("@cashloanbillid"))
                {
                    ht.Add("@cashloanbillid", obj.Cashloanbillid);
                }
            }
            if (obj.Describe != null && obj.Describe.Trim().Length > 0)
            {
        ...
        private static string getQueryString(string front, string columns, string LinkTable, string terms)
        {
            ...
            string sql = @"select {0} a.id as id,a.cashloanbillid as cashloanbillid,a.describe as describe,a.amount as amount,a.del as del,a.projectid as projectid,a.projectcode as projectcode,a.createip as createip,a.createdate as createdate,a.userid as userid,a.username as username,a.userdepartmentid as userdepartmentid,a.userdepartmentname as userdepartmentname,a.userextensioncode as userextensioncode,a.reimbursedate as reimbursedate {1} from media_cashloanitem as a {2} where 1=1 {3} ";
        ...
        public static DataTable QueryInfoByObj(CashloanitemInfo obj, string terms, Hashtable ht)
        {
            if (ht == null)
            {
                ht = new Hashtable();
        ...
        public static DataTable QueryInfoByObj(CashloanitemInfo obj, string terms, params SqlParameter[] param)
        {
            if (terms == null)
            {
                terms = string.Empty;
        ...
        #endregion
        #region load
        public static CashloanitemInfo Load(int id)
        {
            DataTable dt = null;
        ...
        public static CashloanitemInfo Load(int id, SqlTransaction trans)
        {
            DataTable dt = null;
        ...
        public static CashloanitemInfo setObject(DataRow dr)
        {
            CashloanitemInfo obj = new CashloanitemInfo();
            if (dr.Table.Columns.Contains("id") && dr["id"] != DBNull.Value)//id
            {
                obj.Id = Convert.ToInt32(dr["id"]);
            }
            if (dr.Table.Columns.Contains("cashloanbillid") && dr["cashloanbillid"] != DBNull.Value)//cashloanbillid
            {
                obj.Cashloanbillid = Convert.ToInt32(dr["cashloanbillid"]);
            }
