...
{
    public class CashloanbillDataProvider
    {
        #region 构造函数
        public CashloanbillDataProvider()
        {
        }
        #endregion
        #region 插入
        //插入字符串
        private static string strinsert(CashloanbillInfo obj, ref List<SqlParameter> ht)
        {
            if (ht == null)
            {
                ht = new List<SqlParameter>();
            }
            string sql = @"insert into media_cashloanbill (projectid,projectcode,createip,createdate,userid,username,userdepartmentid,userdepartmentname,userextensioncode,reimbursedate,del,status) values (@projectid,@projectcode,@createip,@createdate,@userid,@username,@userdepartmentid,@userdepartmentname,@userextensioncode,@reimbursedate,@del,@status);select @@IDENTITY as rowNum;";
        ...
        //插入一条记录
        public static int insertinfo(CashloanbillInfo obj, SqlTransaction trans)
        {
            if (obj == null)
            {
                return 0;
        ...
        //插入一条记录
        public static int insertinfo(CashloanbillInfo obj)
        {
            if (obj == null)
            {
                return 0;
        ...
        {
            int rows = 0;
            string sql = "delete media_cashloanbill where cashloanbillid=@id";
        ...
        #region 更新
        //更新sql
        public static string getUpdateString(CashloanbillInfo objTerm, CashloanbillInfo Objupdate, string term, ref List<SqlParameter> ht, params SqlParameter[] param)
        {
            string sql = "update media_cashloanbill set projectid=@projectid,projectcode=@projectcode,createip=@createip,createdate=@createdate,userid=@userid,username=@username,userdepartmentid=@userdepartmentid,userdepartmentname=@userdepartmentname,userextensioncode=@userextensioncode,reimbursedate=@reimbursedate,del=@del,status=@status where 1=1 ";
            SqlParameter param_projectid = new SqlParameter("@projectid", SqlDbType.Int, 4);
        ...
        //更新操作
        public static bool updateInfo(SqlTransaction trans, CashloanbillInfo objterm, CashloanbillInfo Objupdate, string term, params SqlParameter[] param)
        {
            if (Objupdate == null)
            {
                return false;
        ...
        //更新操作
        public static bool updateInfo(CashloanbillInfo objterm, CashloanbillInfo Objupdate, string term, params SqlParameter[] param)
        {
            if (Objupdate == null)
            {
                return false;
        ...
        private static string getTerms(CashloanbillInfo obj, ref List<SqlParameter> ht)
        {
        ...
        #endregion
        #region 查询
        private static string getQueryTerms(CashloanbillInfo obj, ref Hashtable ht)
        {
            string term = string.Empty;
            if (obj.Cashloanbillid > 0)//cashloanbillid
            {
        ...
        private static string getQueryString(string front, string columns, string LinkTable, string terms)
        {
            ...
            string sql = @"select {0} a.cashloanbillid as cashloanbillid,a.projectid as projectid,a.projectcode as projectcode,a.createip as createip,a.createdate as createdate,a.userid as userid,a.username as username,a.userdepartmentid as userdepartmentid,a.userdepartmentname as userdepartmentname,a.userextensioncode as userextensioncode,a.reimbursedate as reimbursedate,a.del as del,a.status as status {1} from media_cashloanbill as a {2} where 1=1 {3} ";
        ...
        public static DataTable QueryInfoByObj(CashloanbillInfo obj, string terms, Hashtable ht)
        {
            if (ht == null)
            {
                ht = new Hashtable();
        ...
        public static DataTable QueryInfoByObj(CashloanbillInfo obj, string terms, params SqlParameter[] param)
        {
            if (terms == null)
            {
                terms = string.Empty;
        ...
        #endregion
        #region load
        public static CashloanbillInfo Load(int id)
        {
            DataTable dt = null;
            dt = QueryInfo(" and a.cashloanbillid=@id ", new SqlParameter("@id", id));
        ...
        public static CashloanbillInfo Load(int id, SqlTransaction trans)
        {
            DataTable dt = null;
            dt = QueryInfo(trans, " and a.cashloanbillid=@id ", new SqlParameter("@id", id));
            if (dt != null && dt.Rows.Count > 0)
            {
                return setObject(dt.Rows[0]);
            }
        ...
        public static CashloanbillInfo setObject(DataRow dr)
        {
            CashloanbillInfo obj = new CashloanbillInfo();
