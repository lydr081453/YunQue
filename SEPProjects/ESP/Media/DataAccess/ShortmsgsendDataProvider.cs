...
namespace ESP.Media.DataAccess
{
    public class ShortmsgsendDataProvider
    {
        ...
        public static ShortmsgsendInfo setObject(DataRow dr)
        {
            ...
            if (dr.Table.Columns.Contains("shortmsgid") && dr["shortmsgid"] != DBNull.Value)//短信息id
            {
                obj.Shortmsgid = Convert.ToInt32(dr["shortmsgid"]);
            }
            if (dr.Table.Columns.Contains("recvuserid") && dr["recvuserid"] != DBNull.Value)//收信人id
            {
                obj.Recvuserid = Convert.ToInt32(dr["recvuserid"]);
            }
            if (dr.Table.Columns.Contains("recvusertype") && dr["recvusertype"] != DBNull.Value)//收信人类型
            {
                obj.Recvusertype = Convert.ToInt32(dr["recvusertype"]);
            }
            if (dr.Table.Columns.Contains("recvphoneno") && dr["recvphoneno"] != DBNull.Value)//接收的电话号码
            {
                obj.Recvphoneno = (dr["recvphoneno"]).ToString();
            }
            if (dr.Table.Columns.Contains("senduserid") && dr["senduserid"] != DBNull.Value)//发送人id
            {
