...
{
    public class MailmsgDataProvider
    {
        #region 构造函数
        public MailmsgDataProvider()
        {
        }
        #endregion
        #region 插入
        //插入字符串
        private static string strinsert(MailmsgInfo obj, ref List<SqlParameter> ht)
        {
            if (ht == null)
            {
                ht = new List<SqlParameter>();
            }
            string sql = @"insert into media_mailmsg (subject,body,createid,createdate,senddate,attachmentspath,del) values (@subject,@body,@createid,@createdate,@senddate,@attachmentspath,@del);select @@IDENTITY as rowNum;";
            ...
            SqlParameter param_Body = new SqlParameter("@Body", SqlDbType.NVarChar, 2000);
            ...
            SqlParameter param_Attachmentspath = new SqlParameter("@Attachmentspath", SqlDbType.NVarChar, 1000);
            param_Attachmentspath.Value = obj.Attachmentspath;
            ht.Add(param_Attachmentspath);
        ...
        //插入一条记录
        public static int insertinfo(MailmsgInfo obj, SqlTransaction trans)
        {
            if (obj == null)
            {
                return 0;
        ...
        //插入一条记录
        public static int insertinfo(MailmsgInfo obj)
        {
            if (obj == null)
            {
                return 0;
        ...
        {
            int rows = 0;
            string sql = "delete media_mailmsg where id=@id";
        ...
        #region 更新
        //更新sql
        public static string getUpdateString(MailmsgInfo objTerm, MailmsgInfo Objupdate, string term, ref List<SqlParameter> ht, params SqlParameter[] param)
        {
            string sql = "update media_mailmsg set subject=@subject,body=@body,createid=@createid,createdate=@createdate,senddate=@senddate,attachmentspath=@attachmentspath,del=@del where 1=1 ";
            SqlParameter param_subject = new SqlParameter("@subject", SqlDbType.NVarChar, 100);
            ...
            ht.Add(param_senddate);
            SqlParameter param_attachmentspath = new SqlParameter("@attachmentspath", SqlDbType.NVarChar, 1000);
            param_attachmentspath.Value = Objupdate.Attachmentspath;
            ht.Add(param_attachmentspath);
            SqlParameter param_del = new SqlParameter("@del", SqlDbType.SmallInt, 2);
        ...
        //更新操作
        public static bool updateInfo(SqlTransaction trans, MailmsgInfo objterm, MailmsgInfo Objupdate, string term, params SqlParameter[] param)
        {
            if (Objupdate == null)
            {
                return false;
        ...
        //更新操作
        public static bool updateInfo(MailmsgInfo objterm, MailmsgInfo Objupdate, string term, params SqlParameter[] param)
        {
            if (Objupdate == null)
            {
                return false;
        ...
        private static string getTerms(MailmsgInfo obj, ref List<SqlParameter> ht)
        {
            ...
            if (obj.Body != null && obj.Body.Trim().Length > 0)
            {
                ...
                {
                    SqlParameter p = new SqlParameter("@body", SqlDbType.NVarChar, 2000);
                    p.Value = obj.Body;
                    ht.Add(p);
                }
            }
            ...
            if (obj.Attachmentspath != null && obj.Attachmentspath.Trim().Length > 0)
            {
                term += " and attachmentspath=@attachmentspath ";
                if (ESP.Media.Access.Utilities.Common.Find(ht, "@attachmentspath") == -1)
                {
                    SqlParameter p = new SqlParameter("@attachmentspath", SqlDbType.NVarChar, 1000);
                    p.Value = obj.Attachmentspath;
                    ht.Add(p);
                }
            }
            if (obj.Del > 0)//删除标记
        ...
        #endregion
        #region 查询
        private static string getQueryTerms(MailmsgInfo obj, ref Hashtable ht)
        {
            string term = string.Empty;
            if (obj.Id > 0)//id
            {
            ...
            if (obj.Attachmentspath != null && obj.Attachmentspath.Trim().Length > 0)
            {
                term += " and a.attachmentspath=@attachmentspath ";
                if (!ht.ContainsKey("@attachmentspath"))
                {
                    ht.Add("@attachmentspath", obj.Attachmentspath);
                }
            }
            if (obj.Del > 0)//删除标记
            {
        ...
        private static string getQueryString(string front, string columns, string LinkTable, string terms)
        {
            ...
            string sql = @"select {0} a.id as id,a.subject as subject,a.body as body,a.createid as createid,a.createdate as createdate,a.senddate as senddate,a.attachmentspath as attachmentspath,a.del as del {1} from media_mailmsg as a {2} where 1=1 {3} ";
        ...
        public static DataTable QueryInfoByObj(MailmsgInfo obj, string terms, Hashtable ht)
        {
            if (ht == null)
            {
                ht = new Hashtable();
        ...
        public static DataTable QueryInfoByObj(MailmsgInfo obj, string terms, params SqlParameter[] param)
        {
            if (terms == null)
            {
                terms = string.Empty;
        ...
        #endregion
        #region load
        public static MailmsgInfo Load(int id)
        {
            DataTable dt = null;
        ...
        public static MailmsgInfo Load(int id, SqlTransaction trans)
        {
            DataTable dt = null;
        ...
        public static MailmsgInfo setObject(DataRow dr)
        {
            MailmsgInfo obj = new MailmsgInfo();
            ...
            if (dr.Table.Columns.Contains("createdate") && dr["createdate"] != DBNull.Value)//创建时间
            {
            ...
            if (dr.Table.Columns.Contains("attachmentspath") && dr["attachmentspath"] != DBNull.Value)//attachmentspath
            {
                obj.Attachmentspath = (dr["attachmentspath"]).ToString();
            }
