...
{
    public class ReportersDataProvider
    {
        #region 构造函数
        public ReportersDataProvider()
        {
        }
        #endregion
        #region 插入
        //插入字符串
        private static string strinsert(ReportersInfo obj, ref List<SqlParameter> ht)
        {
            if (ht == null)
            {
                ht = new List<SqlParameter>();
            }
            string sql = @"insert into media_reporters (currentversion,sn,status,createdbyuserid,createdip,createddate,lastmodifiedbyuserid,lastmodifiedip,lastmodifieddate,reportername,penname,sex,birthday,cardnumber,media_id,attention,reporterlevel,reporterposition,tel_o,tel_h,address_h,postcode_h,character,hobby,marriage,writing,family,usualmobile,backupmobile,fax,qq,msn,emailone,emailtwo,emailthree,unittype,unitname,photo,experience,education,del,cityid,bankname,responsibledomain,paytype,bankcardcode,bankcardname,bankacountname,writingfee,referral,haveinvoice,paystatus,uploadstarttime,uploadendtime,paymentmode,privateremark,cooperatecircs,hometown,othermessagesoftware,remark,cityname,OfficeAddress,OfficePostID) values (@currentversion,@sn,@status,@createdbyuserid,@createdip,@createddate,@lastmodifiedbyuserid,@lastmodifiedip,@lastmodifieddate,@reportername,@penname,@sex,@birthday,@cardnumber,@media_id,@attention,@reporterlevel,@reporterposition,@tel_o,@tel_h,@address_h,@postcode_h,@character,@hobby,@marriage,@writing,@family,@usualmobile,@backupmobile,@fax,@qq,@msn,@emailone,@emailtwo,@emailthree,@unittype,@unitname,@photo,@experience,@education,@del,@cityid,@bankname,@responsibledomain,@paytype,@bankcardcode,@bankcardname,@bankacountname,@writingfee,@referral,@haveinvoice,@paystatus,@uploadstarttime,@uploadendtime,@paymentmode,@privateremark,@cooperatecircs,@hometown,@othermessagesoftware,@remark,@cityname,@OfficeAddress,@OfficePostID);select @@IDENTITY as rowNum;";
            ...
            SqlParameter param_OfficeAddress = new SqlParameter("@OfficeAddress", SqlDbType.NVarChar, 500);
            param_OfficeAddress.Value = obj.OfficeAddress;
            ht.Add(param_OfficeAddress);

            SqlParameter param_OfficePostID = new SqlParameter("@OfficePostID", SqlDbType.NChar, 10);
            param_OfficePostID.Value = obj.OfficePostID;
            ht.Add(param_OfficePostID);
        ...
        //插入一条记录
        public static int insertinfo(ReportersInfo obj, SqlTransaction trans)
        {
            if (obj == null)
            {
                return 0;
        ...
        //插入一条记录
        public static int insertinfo(ReportersInfo obj)
        {
            if (obj == null)
            {
                return 0;
        ...
        public static bool DeleteInfo(int id, SqlTransaction trans)
        {
            ...
            {
                int rows = 0;
                string sql = "delete media_reporters where ReporterID=@id";
        ...
        public static string getUpdateString(ReportersInfo objTerm, ReportersInfo Objupdate, string term, ref List<SqlParameter> ht, params SqlParameter[] param)
        {
            ...
            SqlParameter param_OfficeAddress = new SqlParameter("@OfficeAddress", SqlDbType.NVarChar, 500);
            param_OfficeAddress.Value = Objupdate.OfficeAddress;
            ht.Add(param_OfficeAddress);

            SqlParameter param_OfficePostID = new SqlParameter("@OfficePostID", SqlDbType.NChar, 10);
            param_OfficePostID.Value = Objupdate.OfficePostID;
            ht.Add(param_OfficePostID);
            if (objTerm == null && (term == null || term.Trim().Length <= 0))
            {
                sql += " and ReporterID=@id ";
                if (ESP.Media.Access.Utilities.Common.Find(ht, "@id") == -1)
                {
                    SqlParameter p = new SqlParameter("@id", SqlDbType.Int, 4);
                    p.Value = Objupdate.Reporterid;
                    ht.Add(p);
                }
        ...
        //更新操作
        public static bool updateInfo(SqlTransaction trans, ReportersInfo objterm, ReportersInfo Objupdate, string term, params SqlParameter[] param)
        {
            if (Objupdate == null)
            {
                return false;
        ...
        //更新操作
        public static bool updateInfo(ReportersInfo objterm, ReportersInfo Objupdate, string term, params SqlParameter[] param)
        {
            if (Objupdate == null)
            {
                return false;
        ...
        private static string getTerms(ReportersInfo obj, ref List<SqlParameter> ht)
        {
            ...
            if (obj.OfficeAddress != null && obj.OfficeAddress.Trim().Length > 0)
            {
                term += " and OfficeAddress=@OfficeAddress ";
                if (ESP.Media.Access.Utilities.Common.Find(ht, "@OfficeAddress") == -1)
                {
                    SqlParameter p = new SqlParameter("@OfficeAddress", SqlDbType.NVarChar, 500);
                    p.Value = obj.OfficeAddress;
                    ht.Add(p);
                }
            }

            if (obj.OfficePostID != null && obj.OfficePostID.Trim().Length > 0)
            {
                term += " and OfficePostID=@OfficePostID ";
                if (ESP.Media.Access.Utilities.Common.Find(ht, "@OfficePostID") == -1)
                {
                    SqlParameter p = new SqlParameter("@OfficePostID", SqlDbType.NChar, 10);
                    p.Value = obj.OfficePostID;
                    ht.Add(p);
                }
            }
            return term;
        ...
        #endregion
        #region 查询
        private static string getQueryTerms(ReportersInfo obj, ref Hashtable ht)
        {
            string term = string.Empty;
            if (obj.Reporterid > 0)//ReporterID
            {
            ...
            if (obj.OfficeAddress != null && obj.OfficeAddress.Trim().Length > 0)
            {
                term += " and a.OfficeAddress=@OfficeAddress ";
                if (!ht.ContainsKey("@OfficeAddress"))
                {
                    ht.Add("@OfficeAddress", obj.OfficeAddress);
                }
            }
            ...
            if (obj.OfficePostID != null && obj.OfficePostID.Trim().Length > 0)
            {
                term += " and a.OfficePostID=@OfficePostID ";
                if (!ht.ContainsKey("@OfficePostID"))
                {
                    ht.Add("@OfficePostID", obj.OfficePostID);
                }
            }
            return term;
        }
        ...
        public static DataTable QueryInfo(string terms, params SqlParameter[] param)
        {
            ...
            {
                ESP.Logging.Logger.Add("select reporter is error.", "Media system", ESP.Logging.LogLevel.Information, ex); 
            }
            return dt;
        }
        ...
        public static DataTable QueryInfoByObj(ReportersInfo obj, string terms, Hashtable ht)
        {
            if (ht == null)
            {
                ht = new Hashtable();
        ...
        public static DataTable QueryInfoByObj(ReportersInfo obj, string terms, params SqlParameter[] param)
        {
            if (terms == null)
            {
                terms = string.Empty;
        ...
        #endregion
        #region load
        public static ReportersInfo Load(int id)
        {
            DataTable dt = null;
            dt = QueryInfo(" and a.reporterid=@id ", new SqlParameter("@id", id));
        ...
        public static ReportersInfo Load(int id, SqlTransaction trans)
        {
            DataTable dt = null;
            dt = QueryInfo(trans, " and a.reporterid=@id ", new SqlParameter("@id", id));
            if (dt != null && dt.Rows.Count > 0)
            {
                return setObject(dt.Rows[0]);
            }
        ...
        public static ReportersInfo setObject(DataRow dr)
        {
            ReportersInfo obj = new ReportersInfo();
            ...
            if (dr.Table.Columns.Contains("media_id") && dr["media_id"] != DBNull.Value)//媒体编号
            {
                obj.Media_id = Convert.ToInt32(dr["media_id"]);
            }
            ...
            if (dr.Table.Columns.Contains("OfficeAddress") && dr["OfficeAddress"] != DBNull.Value)//OfficeAddress
            {
                obj.OfficeAddress = (dr["OfficeAddress"]).ToString();
            }
            if (dr.Table.Columns.Contains("OfficePostID") && dr["OfficePostID"] != DBNull.Value)//OfficePostID
            {
                obj.OfficePostID = (dr["OfficePostID"]).ToString();
            }
            return obj;
        }
        #endregion
