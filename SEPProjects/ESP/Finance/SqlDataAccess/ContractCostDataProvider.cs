...
namespace ESP.Finance.DataAccess
{
	...
	/// 数据访问类F_ContractCost。
	/// </summary>
	internal class ContractCostDataProvider : ESP.Finance.IDataAccess.IContractCostDataProvider
	{
		...
		public bool Exists(int ContractCostID)
		{
			...
			strSql.Append("select count(1) from F_ContractCost");
        ...
        ///// 增加一条数据
        ///// </summary>
        //public int Add(F_ContractCost model)
        //{
		...
		/// 增加一条数据
		/// </summary>
		public int Add(ContractCostInfo model)
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("insert into F_ContractCost(");
        ...
        ///// 更新一条数据
        ///// </summary>
        //public int Update(F_ContractCost model)
        //{
		...
		/// 更新一条数据
		/// </summary>
		public int Update(ContractCostInfo model)
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("update F_ContractCost set ");
		...
		public int Delete(int ContractCostID)
		{
			...
			StringBuilder strSql=new StringBuilder();
			strSql.Append("delete F_ContractCost ");
			strSql.Append(" where ContractCostID=@ContractCostID ");
			SqlParameter[] parameters = {
					new SqlParameter("@ContractCostID", SqlDbType.Int,4)};
			parameters[0].Value = ContractCostID;
        ...
        ///// 得到一个对象实体
        ///// </summary>
        //public F_ContractCost GetModel(int ContractCostID)
        //{
		...
		/// 得到一个对象实体
		/// </summary>
		public ContractCostInfo GetModel(int ContractCostID)
		{
			
			StringBuilder strSql=new StringBuilder();
            strSql.Append("select  top 1 ContractCostID,ContractID,ProjectID,ProjectCode,Description,Cost,Remark,CostTypeID from F_ContractCost ");
			strSql.Append(" where ContractCostID=@ContractCostID ");
			SqlParameter[] parameters = {
					new SqlParameter("@ContractCostID", SqlDbType.Int,4)};
			parameters[0].Value = ContractCostID;

            return CBO.FillObject<ContractCostInfo>(DbHelperSQL.Query(strSql.ToString(), parameters));
		}
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select sum(cost) from F_ContractCost");
        ...
        public IList<ContractCostInfo> GetList(string term, List<SqlParameter> param)
        {
		...
		/// 获得数据列表
		/// </summary>
        public IList<ContractCostInfo> GetList(string term, List<SqlParameter> param,SqlTransaction trans)
		{
			StringBuilder strSql=new StringBuilder();
            strSql.Append("select ContractCostID,ContractID,ProjectID,ProjectCode,Description,Cost,Remark,CostTypeID ");
			strSql.Append(" FROM F_ContractCost ");
            if (!string.IsNullOrEmpty(term))
            {
                strSql.Append(" where " + term);
            }
            return CBO.FillCollection<ContractCostInfo>(DbHelperSQL.Query(strSql.ToString(),trans,(param == null ? null : param.ToArray())));
		}

        public IList<ESP.Finance.Entity.ContractCostInfo> GetListByProject(int projectId, string term, List<System.Data.SqlClient.SqlParameter> param)
        {
            return GetListByProject(projectId, term, param, null);
        }

        public IList<ESP.Finance.Entity.ContractCostInfo> GetListByProject(int projectId, string term, List<System.Data.SqlClient.SqlParameter> param,SqlTransaction trans)
        {
            if (string.IsNullOrEmpty(term))
            {
                term = " 1=1 ";
