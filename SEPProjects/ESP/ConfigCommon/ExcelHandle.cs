...
namespace ESP.ConfigCommon
{
    ...
    public class ExcelHandle
    {
        ...
        {
            return CurSheet.get_Range(cellname, cellname).Value2 == null ? "" : CurSheet.get_Range(cellname, cellname).Value2.ToString();
        }
        ...
        private static int LtoN(string strLetter)
        {
            int intRtn = 0;
        ...
        private static string NtoL(int intNumber)
        {
            if (intNumber == 26)
                return "Z";
            if (intNumber > 702)
