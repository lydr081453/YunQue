...
jQuery.Autocompleter.Select = function (options, input, select) {
	...
	var list = jQuery("<ul>").appendTo(element).mouseover( function(event) {if(target(event)!=null){
		active = jQuery("li", list).removeClass(CLASSES.ACTIVE).index(target(event));
		jQuery(target(event)).addClass(CLASSES.ACTIVE);}
	}).mouseout( function(event) {if(target(event)!=null){
...
//        {
//            var element = event.target;
//            while((element.tagName != null) && (element.tagName != "LI"))
//                element = element.parentNode;
//            return element;
//        }
//        else
