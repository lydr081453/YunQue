...
namespace FrameWork.Web.Public
{
	...
	public partial class UserChoice : ESP.Web.UI.PageBase
	{
		...
		private void FillDg()
		{
            ...
            //if(QueryStr.Text.Trim()!="")
            //{
            //    sql = "select top " + ConfigManager.MaxRecordCount + " * from F_Employee where UserCode like '%" + Utils.DoubleQuote(QueryStr.Text.Trim()) + "%' " +
            //        " or UserName like '%" + QueryStr.Text.Trim() + "%'";
            //}
            //else
            //{
            //    sql = "select top " + ConfigManager.MaxRecordCount + " * from F_Employee";
            //}
