...
{
    public partial class MarriedStatistics : ESP.Web.UI.PageBase
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            #region AjaxProRegister
            AjaxPro.Utility.RegisterTypeForAjax(typeof(MarriedStatistics));
            #endregion
            if (!IsPostBack)
            {
                DepartmentDataBind();
        ...
        {
          //  List<ESP.HumanResource.Entity.StatisticsInfo> list = StatisticsManager.getListForMarried();
            ...
            if (ESP.Framework.BusinessLogic.PermissionManager.HasModulePermission("001", this.ModuleInfo.ModuleID, this.UserID))
            {
                ...
                list = ESP.HumanResource.BusinessLogic.StatisticsManager.getListForMarried(UserInfo, depids, "", true);
            }
            else
            {
                list = ESP.HumanResource.BusinessLogic.StatisticsManager.getListForMarried(UserInfo, depids, "", false);
            }
            ...
            for (int i = 0; i < list.Count; i++)
            {
                ...
                {
                    list[i].MarriedRateByDep = (list[i].MarriedNumByDep / countGroup * 100);
                    list[i].MarriedRateByCo = (list[i].MarriedNumByDep / countCo * 100);
                    list[i].UnmarriedRateByDep = (list[i].UnmarriedNumByDep / countGroup * 100);
                    list[i].UnmarriedRateByCo = (list[i].UnmarriedNumByDep / countCo * 100);
                    list[i].Unknow2RateByDep = (list[i].Unknow2NumByDep / countGroup * 100);
                    list[i].Unknow2RateByCo = (list[i].Unknow2NumByDep / countCo * 100);
                }
