...
{
    public partial class InsuranceCompensationList : ESP.Web.UI.PageBase
    {
        ...
        protected void listBind()
        {
            ...
            {
                strCondition += string.Format(" (u.lastnamecn+u.firstnamecn like '%{0}%' or u.username like '%{0}%') ", txtName.Text.Trim());
            }
            List<ESP.HumanResource.Entity.PayInsuranceInfo> list = null;
            if (ESP.Framework.BusinessLogic.PermissionManager.HasModulePermission("001", this.ModuleInfo.ModuleID, this.UserID))
            {
                list = ESP.HumanResource.BusinessLogic.PayInsuranceManager.GetModelList(strCondition);
            }
            else
            {
                list = ESP.HumanResource.BusinessLogic.PayInsuranceManager.GetUserModelList(UserInfo, strCondition);
            }
            gvE.DataSource = list;
            gvE.DataBind();
