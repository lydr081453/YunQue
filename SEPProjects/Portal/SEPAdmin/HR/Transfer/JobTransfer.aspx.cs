...
public partial class Transfer_JobTransfer : ESP.Web.UI.PageBase
{
    ...
    private void ListBind()
    {
        ...
        if (empid.Length > 0)
        {
            ...
            strWhere += (string.Format(" and newCompanyID in ({0})", empid));
        }
        DataSet ds = TransferManager.GetList(strWhere);
    ...
    protected void gvList_RowCommand(object sender, GridViewCommandEventArgs e)
    {
        if (e.CommandName == "Del")
        {
            ...
            ESP.HumanResource.Entity.TransferInfo model = TransferManager.GetModel(id);
            TransferManager.Delete(model, LogManager.GetLogModel(UserInfo.Username + "删除了" + model.sysUserName + "的部门调整登记", UserInfo.UserID, UserInfo.Username, model.sysid, model.sysUserName, Status.Log));
